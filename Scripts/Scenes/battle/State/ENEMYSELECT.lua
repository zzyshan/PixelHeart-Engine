...
local function CreateMenuact(acts, startPos)
    ...
    for i, text in ipairs(acts) do
        ...
        local actText = typer.New("[skip]* " .. text, {x, y}, 3, {voice = "uifont.wav"})
        actText:SetScale(28)
...
function ENEMYSELECT.update(ui)
    ...
    Player.sprite.x = ui.enemie_text[ui.enemieindex].x[1] - 25
    Player.sprite.y = ui.enemie_text[ui.enemieindex].y[1] + 15
