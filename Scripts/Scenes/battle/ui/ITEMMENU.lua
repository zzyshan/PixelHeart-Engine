...
function ITEMMENU.CreateItemTexts(ui)
    ...
    for i = start_index, end_index do
        ...
        local item_text = typer.New(
            "[skip]* " .. item.name,
            {pos.x, pos.y},
            3
        )
        item_text:SetScale(28)
        table.insert(ui.itemtext, item_text)
    end
    
    -- 创建页码文本
    ITEMMENU.page_text = typer.New(
        "[skip]PAGE " .. ITEMMENU.current_page .. "/" .. ITEMMENU.total_pages,
        {385, 335},
        3
    )
    ITEMMENU.page_text:SetxCharSpacing(1)
    ITEMMENU.page_text:SetScale(28)
end

-- 修正后的导航逻辑
function ITEMMENU.update(ui)
    if Player.inventory[1] == nil then STATE("ACTIONSELECT") return end
    -- 计算当前页物品数量
    local start_index = ITEMMENU.items_per_page * (ITEMMENU.current_page - 1) + 1
    local end_index = math.min(start_index + ITEMMENU.items_per_page - 1, #Player.inventory)
    ...
    if page_item_count > 0 then
        Player.sprite.x = ui.itemtext[ITEMMENU.itemindex].x[1] - 25
        Player.sprite.y = ui.itemtext[ITEMMENU.itemindex].y[1] + 15
    end
